 ...
 * the next Halloween is a one-line change in the data module. Days carrying an
 * entry are buttons; picking one shows its detail below the grid, which keeps
 * the whole month readable on a phone without a popover.
 */
  ...
  return (
    <section id="calendar">
      <div className="hallo-wrap">
        <h2>October in Senoia</h2>
        <p className="hallo-lede">
          Every date on the calendar, from sign-ups to the last porch light on Halloween night.
        </p>

        <ul className="hallo-legend">
          {usedKinds.map((kind) => (
            <li key={kind}>
              <span className="hallo-cal-dot" style={{ backgroundColor: CALENDAR_KINDS[kind].color }} aria-hidden="true" />
        ...
        <div className="hallo-cal-grid" role="grid" aria-label={`October ${event.year}`}>
          ...
          {days.map((day) => {
            ...
            const label = `${format(day, 'MMMM d')}${items.length ? `, ${items.length} event${items.length > 1 ? 's' : ''}` : ''}`;
            ...
            return (
              ...
              >
                <span className="hallo-cal-num">{format(day, 'd')}</span>
                <span className="hallo-cal-dots">
                  {items.map((item, i) => (
                    <span
                      key={i}
                      className="hallo-cal-dot"
                      style={{ backgroundColor: CALENDAR_KINDS[item.kind].color }}
                      aria-hidden="true"
                    />
                  ))}
                </span>
